...
class TestBssCals(unittest.TestCase):
    ...
    def testFixHNoPrior(self):
        nsamples = 30
        Href = np.random.random((2, nsamples))
        s_priors = None
        obss = bss_cals.BssCals()
        H =  obss._fix_H(Href, s_priors)
        np.testing.assert_array_almost_equal(H, Href, 6,'Wrong H')
    
    def testFixHPrior(self):
        nsamples = 30
        H = np.random.random((2, nsamples))
        s_prior = np.random.random(nsamples)
        s_priors = {'2': s_prior}
        obss = bss_cals.BssCals()
        
        s_prior = np.linalg.norm(H[1,:]) * s_prior / np.linalg.norm(s_prior)
        Href = np.stack((H[0,:], s_prior))
        Href = Href / np.linalg.norm(Href)

        H =  obss._fix_H(H, s_priors)
    ...
    def testMethodFatorizationPriorBound(self):
        ...
        mask = 1
        params = {'max_iters': 10, 'max_sources': 2 }
        t2_bounds = {'1': [0, 0.15], '2': [1.5, 2.5]}
        s_prior = {'2': Scsf}
        ...
        self.assertAlmostEqual(result.rel_error[r,c,s], 0, 2, 'Unexpected S0 values')
